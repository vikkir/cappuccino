...
{
    id sourceListDataSource @accessors;
    var implementedSourceListDataSourceMethods;
}

- (void)setSourceListDataSource:(id)aDataSource
{
    sourceListDataSource = aDataSource;

    if ([sourceListDataSource respondsToSelector:@selector(sourceList:itemHasBadge:)])
        implementedSourceListDataSourceMethods |= CPSourceListDataSource_sourceList_itemHasBadge_;
    if ([sourceListDataSource respondsToSelector:@selector(sourceList:badgeValueForItem:)])
        implementedSourceListDataSourceMethods |= CPSourceListDataSource_sourceList_badgeValueForItem_;
    if ([sourceListDataSource respondsToSelector:@selector(sourceList:badgeBackgroundColorForItem:)])
        implementedSourceListDataSourceMethods |= CPSourceListDataSource_sourceList_badgeBackgroundColorForItem_;
    if ([sourceListDataSource respondsToSelector:@selector(sourceList:badgeTextColorForItem:)])
...
- (BOOL)itemHasBadge:(id)item
{
    if (implementedSourceListDataSourceMethods & CPSourceListDataSource_sourceList_itemHasBadge_)
        return [sourceListDataSource sourceList:self itemHasBadge:item];
...
- (CPInteger)badgeValueForItem:(id)item
{
    if ([self itemHasBadge:item] && implementedSourceListDataSourceMethods & CPSourceListDataSource_sourceList_badgeValueForItem_)
        return [sourceListDataSource sourceList:self badgeValueForItem:item];
...
- (void)drawRow:(CPInteger)rowIndex clipRect:(CGRect)clipRect
{
    ...
    {
        var columnIndex = [self columnWithIdentifier:@"NameColumn"],
...
- (void)drawBadgeForRow:(CPInteger)rowIndex inRect:(CGRect)badgeFrame
{
    ...
    else
    {
        ...
        {
            backgroundColor = [sourceListDataSource sourceList:self badgeBackgroundColorForItem:rowItem];
        ...
        {
            textColor = [sourceListDataSource sourceList:self badgeTextColorForItem:rowItem];
